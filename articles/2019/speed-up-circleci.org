...
* Playing to the strenghts of the platform
  ...
  the workspace in our PR workflow.]. We changed our test jobs to
  launch auxiliary services using CircleCI's docker executor native
  [[https://circleci.com/docs/2.0/configuration-reference/#docker--machine--macosexecutor][service container support]] (rather than =docker-compose=), and run =tox=
  from the main container to install dependencies and run tests. This
